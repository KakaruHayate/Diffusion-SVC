...
class MultiWindowDiscriminator(nn.Module):
    ...
    def __init__(self,
        ...
        for time_length, freq_length, conv_channel in zip(time_lengths, freq_lengths, conv_channels):
            conv_layer = [
                DiscriminatorFactory(np.abs(time_length),  freq_length, conv_channel, conv_in_channels),
            ] # 1d 
            self.conv_layers += conv_layer
...
class MultibandFrequencyDiscriminator(nn.Module):
    def __init__(self,
                 ...
                 multi_channels=[[87, 87, 87, 87, 87 ], [87, 87, 87, 87, 87], [87,87, 87, 87, 87]], n_mel=128
                 ):
        super(MultibandFrequencyDiscriminator, self).__init__()

        self.n_mel = n_mel
        self.time_lengths = time_lengths
        self.multi_win_discriminator_low = MultiWindowDiscriminator(time_lengths, freq_lengths, multi_channels[0], conv_in_channels = int(self.n_mel / 2))
        self.multi_win_discriminator_middle = MultiWindowDiscriminator(time_lengths, freq_lengths, multi_channels[1], conv_in_channels = int(self.n_mel / 2))
        self.multi_win_discriminator_high = MultiWindowDiscriminator(time_lengths, freq_lengths, multi_channels[2], conv_in_channels = int(self.n_mel / 2))
    ...
    def forward(self, x, x_len, y=None, random_N=[]):
        ...
        a = int(self.n_mel / 2)
        b1 = int(self.n_mel / 4)
        b2 = int(self.n_mel * 3 / 4)
        base_mel = x[:,:,:self.n_mel]
        xa = base_mel[:,:,:a]
        xb = base_mel[:,:, b1:b2]
        xc = base_mel[:,:,a:self.n_mel]
        if y != None:
            y_mel = y[:, :, :self.n_mel]
            ya = y_mel[:,:,:a]
            yb = y_mel[:,:, b1:b2]
            yc = y_mel[:,:,a:self.n_mel]
        else:
            ya = yb = yc = None
...
class Discriminator(nn.Module):
    def __init__(self, n_mel):
        ...
        self.discriminator = MultibandFrequencyDiscriminator(n_mel=n_mel)

    def forward(self, x, x_len=None, y=None, random_N=[]):
        if x_len==None:
            x_len = torch.full((x.size(0),), x.size(1), dtype=torch.long)
        return self.discriminator(x, x_len, y, random_N)


if __name__ == "__main__":
    inputs = torch.randn(4, 1600, 128).cuda()
    tgt = torch.randn(4, 1600, 128).cuda()
